...
namespace Nazio_LT.Tools.NTween
{
    public static class NTweenExtensions
    {
        ...
        public static NTweener NTRotateTo(Transform transform, Quaternion target, float duration) => NTween.NTRotateTo(transform, target, duration);

        public static NTweener NTRotateTo(Transform transform, Vector3 eulerTarget, float duration) => NTween.NTRotateTo(transform, eulerTarget, duration);

        #endregion

        #region NTRotate

        public static NTweener NTRotate(Transform transform, Quaternion delta, float duration) => NTween.NTRotate(transform, delta, duration);

        public static NTweener NTRotate(Transform transform, Vector3 eulerDelta, float duration) => NTween.NTRotate(transform, eulerDelta, duration);

        #endregion

        #region NTScaleTo

        public static NTweener NTScaleTo(Transform transform, Vector3 target, float duration) => NTween.NTScaleTo(transform, target, duration);
